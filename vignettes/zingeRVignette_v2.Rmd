...
# t test: in progress. still working on a wrapper function for this.
baseMean=mcols(dse)$baseMean
hlp = getTResults(res, weights, baseMean)
sum(hlp$padj<=0.05, na.rm=TRUE)
mean(simDataIslam$indDE %in% which(hlp$padj<=0.05)) #TPR
mean(which(hlp$padj<=0.05) %in% simDataIslam$indNonDE) #FDP
```
