...
The `estimateWeightedDispersions` function internally first estimates zingeR posterior probabilities using the `zeroWeightsLS` function above and then propagates these weights in an adapted version of the edgeR `estimateDisp` function to estimate negative binomial dispersions conditional on the zero-inflation weights.
Note that in the analysis it is important to be consistent with the normalization procedure to use. The default normalization procedure in the `estimateWeightedDispersions` function is TMM, since this is the default procedure for edgeR. In a next section, we show a zero-inflated NB analysis using DESeq2 where we re-estimate the weights using the proper DESeq2 normalization. The user can also input normalization factors from any normalization procedure of interest by using the `normFactors` argument in `zeroWeightsLS` or `estimateWeightedDispersions`.
...
#weightsDESeq2=zeroWeightsLS(counts=simDataIslam$counts, design=design, maxit=20,plotW=FALSE, normalization="DESeq2_poscounts")
#ask for input from Mike Love. If a new version of DESeq2 will be available with a weights argument we should recycle this, and only adjust the null distribution from Gaussian to t-distribution.
```
