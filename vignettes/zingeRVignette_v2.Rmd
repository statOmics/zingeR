...
For the zingeR\_DESeq2 analysis we re-estimate the weights using the positive counts normalization recently implemented in DESeq2 and first developed as part of the `phyloseq` Bioconductor package. The weights should then be provided to the DESeq object using the `assays` accessor function, after which a conventional DESeq2 analysis follows. Note that we allow for default shrinkage of the fold changes by specifying `betaPrior=TRUE`.
...
res = results(dse)
# t test: will be updated to a more convenient wrapper function soon.
baseMean=mcols(dse)$baseMean
hlp = getTResults(res, weights, baseMean, p=2)
hist(hlp$pval)
