...
weights=zeroWeights(counts=simDataIslam$counts, design=design, maxit=3,plotW=TRUE)

# do a zero-inflated edgeR analysis
library(edgeR)
d=DGEList(simDataIslam$counts)
d=calcNormFactors(d)
d=estimateWeightedDispersions(d,design, maxit=3)
plotBCV(d)
fit=glmFit(d,design)
...
mean(simDataIslam$indDE%in%which(lrtFilter$table$padjFilter<=0.05))
mean(which(lrtFilter$table$padjFilter<=0.05)%in%simDataIslam$indNonDE)

# do a zero-inflated limma analysis
library(limma)
d=DGEList(simDataIslam$counts)
nf=calcNormFactors(simDataIslam$counts)
y=weightedVoom(d,design, nf=nf, maxit=3)
...
mean(simDataIslam$indDE%in%which(ttFiltered$padjFilter<=0.05))
mean(which(ttFiltered$padjFilter<=0.05)%in%simDataIslam$indNonDE)
``` 
